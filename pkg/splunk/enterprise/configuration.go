...
func getSmartstoreConfigMap(client splcommon.ControllerClient, cr splcommon.MetaObject, spec *enterprisev1.CommonSplunkSpec, instanceType InstanceType) (bool, *corev1.ConfigMap, string) {
	...
	} else if cr.GetObjectKind().GroupVersionKind().Kind == "IndexerCluster" && spec.IndexerClusterRef.Name != "" {
		// sgontla: Ideally we should be fetchign the Kind from the IndexerClusterRef, but looks like it is loosely defined
		// In case of indexer, for now, using the indexer own Kind
		smartStoreConfigMapName = GetSplunkSmartstoreConfigMapName(spec.IndexerClusterRef.Name, cr.GetObjectKind().GroupVersionKind().Kind)
	}

	if smartStoreConfigMapName != "" {
		namespacedName := types.NamespacedName{Namespace: cr.GetNamespace(), Name: smartStoreConfigMapName}
		ok, configMap := splctrl.CheckIfConfigMapExists(client, namespacedName)
...
func updateSplunkPodTemplateWithConfig(client splcommon.ControllerClient, podTemplateSpec *corev1.PodTemplateSpec, cr splcommon.MetaObject, spec *enterprisev1.CommonSplunkSpec, instanceType InstanceType, extraEnv []corev1.EnvVar, statefulSetSecretName string) {
	...
	if ok, smartstoreConfigMap, smartStoreConfigMapName := getSmartstoreConfigMap(client, cr, spec, instanceType); ok {
...
func checkIfVolumeExists(volumeList []enterprisev1.VolumeSpec, volName string) (int, error) {
	...
	return 0, fmt.Errorf("Volume: %s, doesn't exist", volName)
}

func areRemoteVolumeKeysChanged(client splcommon.ControllerClient, cr splcommon.MetaObject, spec *enterprisev1.CommonSplunkSpec, instanceType InstanceType, smartstore *enterprisev1.SmartStoreSpec, retError *error) bool {

	ok, smartstoreConfigMap, _ := getSmartstoreConfigMap(client, cr, spec, instanceType)
	...
	for _, volume := range volList {
		...
		// detected config changed, and proceeded, instead of getting into this path
		if !ok {
			return true
		}
...
func ValidateSplunkSmartstoreSpec(smartstore *enterprisev1.SmartStoreSpec) error {
	...
	// When volName is configured, bucket remote path also should be configured
...
func GetServerConfigEntries(serverConf *enterprisev1.ServerConfSpec) string {
	var serverConfIni string
	cacheManagerConf := serverConf.CacheManager
